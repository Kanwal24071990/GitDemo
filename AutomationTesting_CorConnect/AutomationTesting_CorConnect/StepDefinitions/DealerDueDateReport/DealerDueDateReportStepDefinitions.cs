...
namespace AutomationTesting_CorConnect.StepDefinitions.DealerDueDateReport
{
    [Binding]
    [Scope(Feature = "DealerDueDateReport")]
    internal class DealerDueDateReportStepDefinitions : DriverBuilderClass
    {
        DealerDueDateReportPage DDUDRPage;

        [When(@"Search by DateRange value ""([^""]*)""")]
        public void WhenSearchByDateRangeValue(string DateRangeValue)
        {
            DDUDRPage = new DealerDueDateReportPage(driver);
            DDUDRPage.SelectValueTableDropDown(FieldNames.DateRange, DateRangeValue);
            DDUDRPage.LoadDataOnGrid();
        ...
        {
            if (DDUDRPage.IsAnyDataOnGrid())
            {
                string gridCount = DDUDRPage.GetPageCounterTotal();
                switch (DateRangeValue)
                {
                    case "Last 7 days":
                        int DateRangeLast7Days = DealerDueDateReportUtils.GetCountByDateRange(DateRangeValue, 0);
                        Assert.AreEqual(DateRangeLast7Days, int.Parse(gridCount), "Last 7 days count mismatch");
                        break;
                    case "Last 14 days":
                        int DateRangeLast14Days = DealerDueDateReportUtils.GetCountByDateRange(DateRangeValue, 0);
                        Assert.AreEqual(DateRangeLast14Days, int.Parse(gridCount), "Last 14 days count mismatch");
                        break;
                    case "Last 185 days":
                        int DateRangeLast185Days = DealerDueDateReportUtils.GetCountByDateRange(DateRangeValue, 0);
                        Assert.AreEqual(DateRangeLast185Days, int.Parse(gridCount), "Last 185 days count mismatch");
                        break;
                    case "Current month":
                        int DateRangeCurrentMonth = DealerDueDateReportUtils.GetCountByDateRange(DateRangeValue, 0);
                        Assert.AreEqual(DateRangeCurrentMonth, int.Parse(gridCount), "Current Month count mismatch");
                        break;
                    case "Last month":
                        int DateRangeLastmonth = DealerDueDateReportUtils.GetCountByDateRange(DateRangeValue, 0);
                    ...
                    case "Customized date":
                        ...
                        int DateRangeCustomizedDate = DealerDueDateReportUtils.GetCountByDateRange(DateRangeValue, CustomeDate);
