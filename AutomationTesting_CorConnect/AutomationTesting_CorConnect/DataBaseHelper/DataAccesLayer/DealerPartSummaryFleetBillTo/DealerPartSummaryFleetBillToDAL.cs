...
namespace AutomationTesting_CorConnect.DataBaseHelper.DataAccesLayer.DealerPartSummaryFleetBillTo
{
    internal class DealerPartSummaryFleetBillToDAL : BaseDataAccessLayer
    {
            ...
            try
            {
                ...
                if (userType == "ADMIN")
                {
                    query = @"SELECT TOP 1 CONVERT (Date , invoicedate) AS FromDate, Convert (Date, invoicedate) AS ToDate FROM [part_tb] PT INNER JOIN 
                        [partCategoryCode_tb] PC ON PT.categoryCode1Id = PC.partCategoryCodeId RIGHT OUTER JOIN [Invoice_tb] IV INNER JOIN [invoiceSection_tb] ON IV.invoiceId = 
                        invoiceSection_tb.invoiceId INNER JOIN [invoiceLineDetail_tb] as ID ON invoiceSection_tb.invoiceSectionId = ID.invoiceSectionId ON PT.partId = ID.itemId WHERE 
                        ID.lineDetailType = 'P'  AND(ISNULL(ID.accountingDocumentTypeId, 0) = 0 OR ID.accountingDocumentTypeId = (SELECT lookUpId FROM [lookup_tb] WITH (NOLOCK) WHERE 
                        parentLookUpCode = 218 AND lookupcode = 1)) AND ID.rebateType IN(0, 1) ORDER BY invoicedate DESC";
                ...
                else if (userType == "DEALER")
                {
                    query = @"DECLARE @@Userid AS INT; DECLARE @@DealerAccessLocations TABLE (entityDetailId INT PRIMARY KEY); SELECT @@Userid=userid FROM [user_tb] WHERE 
                        ...
                        INNER JOIN [entityDetails_tb] AS C WITH(NOLOCK) ON P.entityDetailId = C.parentEntityDetailId) INSERT INTO @@DealerAccessLocations SELECT entityDetailId FROM 
                        ...
                        CONVERT(DATE, invoicedate) AS ToDate FROM [part_tb] PT INNER JOIN [partCategoryCode_tb] PCC ON PT.categoryCode1Id = PCC.partCategoryCodeId RIGHT OUTER JOIN 
                        ...
                        AND lookupcode = 1)) AND ID.rebateType IN (0, 1) ORDER BY invoiceDate DESC;";
