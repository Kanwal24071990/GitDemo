...
namespace AutomationTesting_CorConnect.DataBaseHelper.DataAccesLayer.POOrders
{
    internal class POOrdersDAL : BaseDataAccessLayer
    {
        internal void GetData(out string FromDate, out string ToDate)
        {
            ...
            try
            {
                ...
                if (userType == "ADMIN")
                {
                    query = @"SELECT TOP 1 CONVERT (Date , t.createdate) AS FromDate, Convert (Date, t.createdate) AS ToDate FROM [transaction_tb] T WITH (NOLOCK) JOIN [transactionPO_tb] PO WITH (NOLOCK) ON (T.transactionId=PO.transactionId) LEFT JOIN [entityDetails_tb] ER WITH (NOLOCK) ON (T.receiverEntityDetailId = ER.entityDetailId AND ER.isActive=1) LEFT JOIN [entityDetails_tb] ES WITH (NOLOCK) ON (T.senderEntityDetailId = ES.entityDetailId AND ES.isActive=1) WHERE T.isActive=1 AND T.requestTypeCode='O' AND T.validationStatus = 1 AND T.isHistorical=0 ORDER BY T.createdate desc;";
        ...
        internal int GetCountByDateRange(string dateRange, int days = 0)
        {
            ...
            if (dateRange == "Last 7 days")
            {
                ...
                query = @"SELECT count (*) FROM transaction_tb t WITH (NOLOCK)  JOIN transactionPO_tb PO WITH (NOLOCK)
                ON t.transactionId=PO.transactionId WHERE t.isActive=1 AND t.requestTypeCode='O' AND t.validationStatus = 1 
                AND t.isHistorical=0 AND t.createdate BETWEEN @fromDateTime AND GETDATE()";
                List<SqlParameter> sp = new()
                {
            ...
            else if (dateRange == "Last month")
            {
                ...
                query = @"SELECT count (*) FROM transaction_tb t WITH (NOLOCK)  JOIN transactionPO_tb PO WITH (NOLOCK)
                ON t.transactionId=PO.transactionId WHERE t.isActive=1 AND t.requestTypeCode='O' AND t.validationStatus = 1 
                AND t.isHistorical=0 AND t.createdate BETWEEN @fromDateTime AND @toDateTime";
