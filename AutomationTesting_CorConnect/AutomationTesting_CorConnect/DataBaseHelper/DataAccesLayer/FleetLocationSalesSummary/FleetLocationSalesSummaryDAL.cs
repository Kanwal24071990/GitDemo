...
namespace AutomationTesting_CorConnect.DataBaseHelper.DataAccesLayer.FleetLocationSalesSummary
{
    internal class FleetLocationSalesSummaryDAL : BaseDataAccessLayer
    {
            ...
            try
            {
                ...
                if (userType == "ADMIN")
                {
                    query = @"SELECT TOP 1 Convert (Date , invoicedate) AS FromDate, Convert (Date , invoicedate) AS 
                        ToDate FROM invoice_tb IV WITH (NOLOCK) INNER JOIN invoiceReport_tb IR WITH (NOLOCK) ON (IV.invoiceId = IR.invoiceId) WHERE 
                ...
                else if (userType == "FLEET")
                {
                    query = @"DECLARE @@Userid as int declare @@FleetAccessLocations table ( entityDetailId INT  primary key ) select @@Userid = userid from user_tb 
                        ...
                        FROM invoice_tb iv with (nolock) inner join @@FleetAccessLocations AE on AE.Entitydetailid = iv.receiverEntityDetailId inner join invoiceReport_tb ir on iv.invoiceId 
                        = ir.invoiceId where iv.isActive = 1 Order by invoicedate desc";
