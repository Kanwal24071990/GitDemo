...
namespace AutomationTesting_CorConnect.DataBaseHelper.DataAccesLayer.POEntry
{
    internal class POEntryDAL : BaseDataAccessLayer
    {
            ...
            try
            {
                ...
                if (userType == "ADMIN")
                {
                    query = @" SELECT    top 1 Convert (Date , transactiondate) as FromDate ,Convert (Date , transactiondate) as ToDate FROM [transaction_tb] tr with (NOLOCK) 
                        where isActive = 1 and validationStatus = 8 and requestTypeCode = 'R' ORDER BY transactiondate DESC";
                ...
                else if (userType == "FLEET")
                {
                    query = @"DECLARE @@Userid as int DECLARE @@FleetAccessLocations table (entityDetailId INT primary key ) SELECT @@Userid = userid FROM [user_tb] where 
                        ...
                        @@UserID  and IsActive =1 AND userRelationships_tb.entityId IS NOT NULL; SELECT top 1 Convert(Date, transactiondate) as FromDate ,Convert(Date, transactiondate) as 
                        ToDate FROM [transaction_tb] tr with (NOLOCK) INNER JOIN  @@FleetAccessLocations AE ON AE.entityDetailId = tr.receiverEntityDetailId WHERE isActive = 1 and 
                        validationStatus = 8 and requestTypeCode = 'R' ORDER BY transactiondate DESC";
        ...
        internal int GetPOSectionCount(string transactionNumber)
        {
            var query = "Select Count(*) from transaction_tb IV INNER JOIN transactionSection_tb IVS ON IV.transactionId=IVS.transactionId where requestTypeCode  IN ('R') and validationStatus=1 AND IV.transactionNumber=@transactionNumber";
        ...
        internal int GetDisputedPOPOQSectionCount(string transactionNumber,string requestType)
        {
            var query = "Select Count(*) from transaction_tb IV INNER JOIN transactionSection_tb IVS ON IV.transactionId=IVS.transactionId where requestTypeCode IN (@requestType) AND IV.transactionNumber=@transactionNumber";
