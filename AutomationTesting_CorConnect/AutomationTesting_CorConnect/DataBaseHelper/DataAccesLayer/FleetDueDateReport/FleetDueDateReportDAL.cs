...
namespace AutomationTesting_CorConnect.DataBaseHelper.DataAccesLayer.FleetDueDateReport
{
    internal class FleetDueDateReportDAL : BaseDataAccessLayer
    {
        internal void GetDateData(out string FromDate, out string ToDate)
        {
            ...
            try
            {
                string query = @"SELECT TOP 1 Convert (Date , arInvoiceDueDate) AS FromDate, Convert 
                    (Date , arInvoiceDueDate) AS ToDate FROM invoice_tb IV WITH (NOLOCK) LEFT JOIN invoiceVariablePaymentTerms_tb ivpt 
                    WITH (NOLOCK) ON (iv.invoiceId = ivpt.invoiceId) ORDER BY arInvoiceDueDate DESC";
