...
namespace AutomationTesting_CorConnect.DataBaseHelper.DataAccesLayer.FleetSummary
{
    internal class FleetSummaryDAL : BaseDataAccessLayer
    {
        internal void GetData(out string FromDate, out string ToDate)
        {
            ...
            try
            {
                ...
                query = @"DECLARE @@sql NVARCHAR(4000) = 'SELECT top 1 Convert (Date , transactions.invTimeStamp) as FromDate ,Convert (Date , transactions.invTimeStamp) as ToDate from [EOP_DATABASE].dbo.invoice AS transactions WITH (NOLOCK) INNER JOIN [EOP_DATABASE].dbo.invreport ON transactions.transactionID = invreport.InvTransID order by transactions.invTimeStamp desc' DECLARE @@FromDate VARCHAR(255), @@ToDate VARCHAR(255); EXEC sys.sp_executesql @@sql, N'@@FromDate VARCHAR(255) OUT, @@ToDate VARCHAR(255) OUT', @@FromDate OUT, @@ToDate OUT;";
